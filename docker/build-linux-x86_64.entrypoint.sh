...
# baked into the image (see docker/build-linux-x86_64.dockerfile).
set -euo pipefail
...
log() { echo "[build-linux-x86_64] $*"; }

# google_fonts ^8.1.0 needs Dart ^3.9 / Flutter >=3.35; the image ships 3.29.3 (Dart 3.7).
# Override only inside Docker so other targets keep the committed pubspec.
ensure_x86_64_pubspec_compat() {
...
EOF
  log "pubspec_overrides.yaml: overrides x86_64 (google_fonts + intl + flutter_plugin_android_lifecycle)"
}

cleanup_x86_64_pubspec_compat() {
  rm -f flutter/pubspec_overrides.yaml
}

trap cleanup_x86_64_pubspec_compat EXIT
...
if [[ ! -d "${VCPKG_ROOT}/installed/${VCPKG_TRIPLET}" ]]; then
  ...
  log "Rebuild the image: ./build.sh linux-x86_64 --rebuild-image"
  exit 1
fi
...
clean_for_force_rebuild() {
  ...
  rm -f bgdesk-*.deb bgdesk-*-${ARCH_SUFFIX}.deb bgdesk-*.rpm bgdesk-*-${ARCH_SUFFIX}.rpm
...
build_rust() {
  ...
  # Single job avoids OOM on Docker Desktop/Colima with limited RAM (incl. qemu).
...
build_rpm() {
  ...
  [[ -d "${bundle}" ]] || {
    log "ERROR: flutter release bundle not found at ${bundle} (run deb build first)"
    exit 1
  }
...
build_appimage() {
  ...
  cp appimage/AppImageBuilder-x86_64.yml "${work}/appimage/"
...
log "BGDesk Linux x86_64 compile starting (version ${VERSION}, FORCE_REBUILD=${FORCE_REBUILD}, BGDESK_CLIENTE=${BGDESK_CLIENTE})"
cleanup_x86_64_pubspec_compat
clean_for_force_rebuild
ensure_x86_64_pubspec_compat
ensure_bridge
