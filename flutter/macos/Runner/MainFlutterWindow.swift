...
private let kBgDeskChannelNamespace = "br.com.boagestao.bgdesk"
...
class MainFlutterWindow: NSWindow {
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "\(kBgDeskChannelNamespace)/macos", binaryMessenger: registrar.messenger)
        channel.setMethodCallHandler({
            (call, result) -> Void in
                ...
                case "setWindowTheme":
                    ...
                    result(nil)
                    break;
                case "hasVisibleWindows":
                    result(hasVisibleAppWindows())
                case "terminate":
                    NSApplication.shared.terminate(self)
                    // Fallback: terminate() may not exit when the Flutter engine is shutting down.
