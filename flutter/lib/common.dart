...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_BGDESK_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
...
/// Whether any application window is currently visible.
Future<bool> hasAnyVisibleWindow() async {
  if (!isDesktop) {
    return false;
  }
  if (isMacOS) {
    if (await RdPlatformChannel.instance.hasVisibleWindows()) {
      return true;
    }
  }
  try {
    ...
    final ids = await rustDeskWinManager.getAllSubWindowIds();
    for (final id in ids) {
      if (!await WindowController.fromWindowId(id).isHidden()) {
        return true;
      }
    }
  } catch (e) {
    debugPrint('hasAnyVisibleWindow error: $e');
  }
  return false;
}
...
Future<void> maybeQuitWhenNoWindows() async {
  ...
  if (isMacOS) {
    ...
    if (await hasAnyVisibleWindow()) {
      debugPrint(
          '[MultiWindowHandler] active windows empty but a window is still visible, skip quit');
      return;
    }
  }
...
const kMacOSPermChannel = kBgDeskHostChannel;
...
const kBoaGestaoWebsiteUrl = 'https://boagestao.com.br';
const kBgdeskDownloadUrl = 'https://bgdesk.com';
const kBoaGestaoPrivacyUrl = 'https://boagestao.com/privacidade/';
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse(kBoaGestaoWebsiteUrl));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
