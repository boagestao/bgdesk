...
const BGDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.boagestao.bgdesk.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(BGDESK_CLIPBOARD_OWNER_FORMAT),
];
...
fn append_owner_marker(mut data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<ClipboardData> {
    data.push(ClipboardData::Special((
        BGDESK_CLIPBOARD_OWNER_FORMAT.to_owned(),
        side.get_owner_data(),
    )));
    data
}
...
impl ClipboardContext {
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == BGDESK_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != BGDESK_CLIPBOARD_OWNER_FORMAT,
