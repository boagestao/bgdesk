...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connection_failed_check_internet", "连接失败。请检查您的网络连接后重试。"),
        ...
        ("allow-insecure-tls-fallback-tip", "默认情况下，对于使用 TLS 的协议，BGDesk 会验证服务器证书。\n启用此选项后，在验证失败时，BGDesk 将转为跳过验证步骤并继续连接。"),
        ("Disable UDP", "禁用 UDP"),
        ("disable-udp-tip", "控制是否仅使用 TCP。\n启用此选项后，BGDesk 将不再使用 UDP 21116，而是使用 TCP 21116。"),
        ("server-oss-not-support-tip", "注意：BGDesk 开源服务器 (OSS server) 不包含此功能。"),
