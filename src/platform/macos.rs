...
fn correct_app_name(s: &str) -> String {
    ...
    // Legacy RustDesk launchd labels/paths must be rewritten before generic name
    // replacements, otherwise "com.carriez.RustDesk" becomes "com.carriez.BGDesk"
    // instead of "com.boagestao.BGDesk" (see get_full_name()).
    s = s.replace(
        "com.carriez.RustDesk_server",
        &format!("{}_server", full_name),
    );
    s = s.replace(
        "com.carriez.RustDesk_service",
        &format!("{}_service", full_name),
    );
    s = s.replace("com.carriez.RustDesk", &full_name);
    // Older BGDesk forks hard-coded a different ORG in update.scpt.
    s = s.replace(
        "br.com.boagestao.BGDesk_server",
        &format!("{}_server", full_name),
    );
    s = s.replace(
        "br.com.boagestao.BGDesk_service",
        &format!("{}_service", full_name),
    );
    s = s.replace("br.com.boagestao.BGDesk", &full_name);
