...
lazy_static::lazy_static! {
    ...
    static ref SUDO_E_PRESERVES_ENV: bool = {
        if !is_root() {
            ...
            let key = format!("__BGDESK_SUDO_E_TEST_{}", std::process::id());
            let val = "1";
...
/// When running as AppImage, install icons and a user application menu entry so the
/// app appears in the desktop environment with the correct icon.
///
/// Icons go to `~/.local/share/icons/hicolor/...` and the shortcut to
/// `~/.local/share/applications/<app>.desktop`, with `Exec` pointing at the AppImage.
pub fn install_appimage_integration() -> ResultType<()> {
    let appimage = match std::env::var("APPIMAGE") {
        Ok(p) if !p.is_empty() && Path::new(&p).is_file() => p,
        _ => return Ok(()),
    };
    let appdir = match std::env::var("APPDIR") {
        Ok(p) if !p.is_empty() && Path::new(&p).is_dir() => p,
        _ => return Ok(()),
    };
    // Avoid writing into root's home when the AppImage is started as a service.
    if is_root() {
        return Ok(());
    }
    let home = match get_home_dir_trusted() {
        Some(p) => p,
        None => {
            log::warn!("Failed to get trusted home directory for AppImage integration");
            return Ok(());
        }
    };

    let display_name = crate::get_app_name();
    let app_id = display_name.to_lowercase();
    let appdir = PathBuf::from(appdir);

    // Copy icons from the AppImage into the user icon theme.
    let mut icons_installed = false;
    let hicolor_src = appdir.join("usr/share/icons/hicolor");
    if hicolor_src.is_dir() {
        if let Ok(size_entries) = std::fs::read_dir(&hicolor_src) {
            for size_entry in size_entries.flatten() {
                let size_name = size_entry.file_name();
                let src_apps = size_entry.path().join("apps");
                if !src_apps.is_dir() {
                    continue;
                }
                let Ok(icon_entries) = std::fs::read_dir(&src_apps) else {
                    continue;
                };
                for icon_entry in icon_entries.flatten() {
                    let icon_name = icon_entry.file_name();
                    let icon_name = icon_name.to_string_lossy();
                    if !icon_name.starts_with(&format!("{app_id}.")) {
                        continue;
                    }
                    ...
                    let dst = dst_dir.join(icon_entry.file_name());
                    match copy_if_changed(&src, &dst) {
                        Ok(true) => icons_installed = true,
                        Ok(false) => {}
                        Err(e) => log::warn!("Failed to install AppImage icon {:?}: {}", src, e),
                    }
                }
            }
        }
    ...
    // Desktop entry pointing at the AppImage path (updates if the file was moved).
    let apps_dir = home.join(".local/share/applications");
    std::fs::create_dir_all(&apps_dir)?;
    let desktop_path = apps_dir.join(format!("{app_id}.desktop"));
    let exec = desktop_exec_path(&appimage);
    let desktop = format!(
        "\
[Desktop Entry]
Name={display_name}
...
"
    ...
    let needs_desktop_write = match std::fs::read_to_string(&desktop_path) {
        Ok(existing) => existing != desktop,
        Err(_) => true,
    };
    if needs_desktop_write {
        std::fs::write(&desktop_path, desktop.as_bytes())?;
        // Mark as trusted on environments that require it (e.g. GNOME/Nautilus).
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = std::fs::metadata(&desktop_path)?.permissions();
            perms.set_mode(0o755);
            std::fs::set_permissions(&desktop_path, perms)?;
        }
        log::info!("Installed AppImage desktop entry at {:?}", desktop_path);
    }

    if icons_installed {
        log::info!("Installed AppImage icons for {app_id} under ~/.local/share/icons");
        let icon_theme_dir = home.join(".local/share/icons/hicolor");
...
fn copy_if_changed(src: &Path, dst: &Path) -> ResultType<bool> {
    if dst.is_file() {
...
/// Quote a path for use in a Desktop Entry `Exec=` field when needed.
